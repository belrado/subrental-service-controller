 ...
 * 미처리 (설치파트너/설치업체/관리자)
 */
class OrderPending extends OrderStatusHandlerAbstract
{
    /**
        ...
        return $this->processOrderUpdate(
            ...
            function ($orderStatusModel, $od_id, $params) {
                ...
                if ($order['od_status'] === '미처리') {
                    throw new CustomException('이미 미처리된 주문입니다.', '0010',
                ...
                if ($order['od_status'] !== '출고완료') {
                  throw new CustomException('출고완료된 주문만 미처리 가능합니다.', '0010',
                    ['od_id' => $od_id, 'params' => $params, 'order status' => $order['od_status']]);
                }
                ...
                if ($orderStatusModel->updateOrderPending($od_id, [
                    'od_status' => $order['od_status'],
                    'mb_id' => $this->loginMb['mb_id']
                ])) {
                    return true;
                } else {
                    throw new CustomException("주문 상태값 변경 (미처리) 실패", '0009', [
                        'od_id' => $od_id
                    ]);
                }
            }
