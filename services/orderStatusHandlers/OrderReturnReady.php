 ...
 * 회수준비중 변경 처리
 */
class OrderReturnReady extends OrderStatusHandlerAbstract
{
    /**
    ...
    public function handle($od_id, array $params, bool $useTransaction): array
    {
        ...
        return $this->processOrderUpdate(
            ...
            function ($orderStatusModel, $od_id, $params) {
                ...
                if (!$order = $this->orderModel->getOrdersByIds($od_id, "gso.od_status, gso.od_id_cancel")) {
                ...
                if ($orderStatusModel->updateOrderReturnReady($od_id, [
                    ...
                    throw new CustomException("주문 상태값 변경 (회수준비중) 실패", '0009', [
                        'od_id' => $od_id
                    ]);
                }
            }
